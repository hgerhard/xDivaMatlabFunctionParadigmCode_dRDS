function pmf_dRDS_correlatedFigure_variableBackgrd( varargin )
%
...
%   so the latest version of PTB should be installed on the machine used to
%   display stimuli. See http://psychtoolbox.org/download/
%
%   The code contains some workarounds for missing functionality in xDiva,
...
%   chunks of code by searching for: "###".
%
%   The code also relies on a few directories being available in
%   xDiva_MatlabFunctions/:
%       HG_Images    contains *.mat files where the images are stored 
...
%          that are supplied as workarounds for missing xDiva functionality
%   
...
if nargin > 0, aSubFxnName = varargin{1}; else error( 'pmf_dRDS_correlatedFigure_variableBackgrd.m called with no arguments' ); end
...
% xDiva did not report the stimulus screen's physical dimensions which are
% necessary for calculating size in visual angle. Secondly, xDiva did not
% yet have video calibration available, so in order to have red & blue
% intensities matched, you need to measure the red gun values by hand and
% find the pixel intensity for red which produces as close of a physical
% luminance value to the maximum intensity of the blue (i.e. 0,0,255 will
% always be less intense than 255,0,0 so you may end up with a match value
% of 110,0,0).
%       To workaround both of these issues, I created 2 text files containing the
% relevant information for a variety of screen settings. In each text file,
% the first column indicates the vertical dimension in pixels, and the
% other values are the result of physically measuring the monitor. Note
% that this is currently hard coded for the SonyTV and new measurements and
% files should be made (and file names on the next two lines editted) if a
% different display is used:
vidFile1 = dir('HG_VideoMeasures/SonyTV_PhysicalDimensions.txt');
vidFile2 = dir('HG_VideoMeasures/SonyTV_RedGunValues.txt');
if ~isempty(vidFile1)
    screenDimData = load('HG_VideoMeasures/SonyTV_PhysicalDimensions.txt');
else
    screenDimData = []; % default values will be used later if file not available
end
if ~isempty(vidFile2)
    screenRedGunData = load('HG_VideoMeasures/SonyTV_RedGunValues.txt');
else
    screenRedGunData = []; % default values will be used later if file not available
end
% ### TEMPORARY xDIVA WORKAROUND ###
...
try
    ...
    function rV = MakeDefinitions
        ...
        'Fix Point'         1.0	'double' {} % ### no xDiva functionality for fixation points yet
        ...
        'Stimulus Extent' 'Fullscreen' 'nominal' {'Fullscreen' 'SquareOnly'} % 'SqrOnBlack' 'SqrOnMean' could be included but are as memory intense as 'Fullscreen'
    ...
    function ValidateParameters
        ...
        if ~isempty(screenDimData)
            width_cm = screenDimData(screenDimData(:,1)==width_pix,2); %### this value would eventually be returned by VMVal('imageWidthCm');
        else
            width_cm = 100; %### default value to be used if file was not found on disk
        end
        ...
        function ValidateContrast
            % Only checks if value requested is within range of 0-100% contrast.
            % It should actually check if requested contrast is not available due to
    ...
    function MakeMovie
        ...
        if needsImFiles
            ...
            framesPerStep = TRVal('nmbFramesPerStep');
            ...
            if ~isempty(screenDimData)
                ...
                width_cm = 100; %### default values to be used if file was not found on disk
                height_cm = 80;
            end
            % ### TEMPORARY xDIVA WORKAROUND: ###
            ...
            % ### TEMPORARY xDIVA WORKAROUND: ###
            if ~isempty(screenRedGunData)
                redMod = screenRedGunData(screenRedGunData(:,1)==width_pix,2)/255;
            else
                redMod = 1; % ### default value to be used if files not found on disk
            end
            ...
            % ---- PTB CODE ----
            usePTB = true; % ### set to false ONLY for debugging/testing
            if usePTB
                tstart = tic;
                ...
                try
                    ...
                    w = window; % choose window or windowOffScreen. Selection will be the place to draw images
                    imcnt = 1;
